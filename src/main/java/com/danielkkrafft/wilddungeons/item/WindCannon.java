...
public class WindCannon extends WDWeapon {
    ...
    protected void configureAnimator(WDItemAnimator animator) {
        this.animator.addAnimation(AnimationList.compress.toString(), 1.0f);
    }

    @Override
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private final GeoItemRenderer<?> renderer = new WindCannonRenderer();
            ...
            if (level instanceof ServerLevel server) {
                if(c >= 20) {
                    ...
                    if(it.isDamageableItem() && enchantmentLevel == 0 && UtilityMethods.RNG( 0, 9) == 0) {
                        ...
                        server.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.GENERIC_EXPLODE, SoundSource.PLAYERS, 1f, 1.5f);
                ...
                animator.playAnimation(this, WindCannon.AnimationList.compress.toString(), stack, player, level);
