...
public class LaserSword extends WDWeapon {
    ...
    public InteractionResultHolder<ItemStack> use(@NotNull Level level, Player player, @NotNull InteractionHand hand) {
        ...
        WildDungeons.getLogger().info("Player started using item {}", player.getItemInHand(hand).getItem().getDescriptionId());
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack stack, int remainingUseDuration) {
        if (livingEntity instanceof Player player) {
            ...
            if (charge == warmUpSeconds *20) setAnimation(AnimationList.charging_up.toString(), stack, player, player.level());
            if (charge == (maxChargeSeconds*20)+(warmUpSeconds *20)) setAnimation(AnimationList.fully_charged.toString(), stack, player, player.level());
        }
    }
