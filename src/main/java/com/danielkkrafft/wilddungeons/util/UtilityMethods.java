...
public final class UtilityMethods {
    ...
    public static <T extends ParticleOptions>void sendParticles(@NotNull ServerLevel server, T t, boolean visible, int count, double x, double y, double z, float dispX, float dispY, float dispZ, float speed) {
        ClientboundLevelParticlesPacket packet = new ClientboundLevelParticlesPacket(t, visible, true, x, y, z, dispX, dispY, dispZ, speed, count);
     ...
     */
    public static void setSpawnPotentials(DungeonRoom room, BaseSpawner spawner, WeightedList<SpawnData> weightedSpawnData) {
        // Create NBT compound to hold spawner data
        ValueOutput spawnerTag = null;
...
//        spawnerTag.put("SpawnPotentials", spawnPotentialsTag); TODO - Fix this thing
        ...
        spawner.load(level.get(), pos.get(), (ValueInput) spawnerTag); //TODO - Fix this whole thing - Proabbly shouldn't be casting ValueInput either...
...
//        spawner.setEntityId(entityType,level.get(), level.get().getRandom(), pos.get());
    ...
    public static WeightedList<Object> createSpawnDataWeightedList(WeightedPool<DungeonRegistration.TargetTemplate> pool) {
        WeightedList.Builder<Object> builder = WeightedList.builder();
