...
public class PermanentHealthBoostPerk extends DungeonPerk {
    ...
    public void onCollect(boolean silent) {
        ...
        players.forEach(wdPlayer -> {
            wdPlayer.getServerPlayer().removeEffect(MobEffects.HEALTH_BOOST);
            wdPlayer.getServerPlayer().addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, -1, this.count-1));
        });
    }
