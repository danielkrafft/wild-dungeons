...
//                    SkelepedeMain newMain = WDEntities.SKELEPEDE.get().create(serverLevel, getSpawnType());
//                    if (newMain != null) {
//                        newMain.absSnapTo(splitSegments.get(0).position().x, splitSegments.get(0).position().y, splitSegments.get(0).position().z);
...
//                SkelepedeSegment segment = WDEntities.SKELEPEDE_SEGMENT.get().create(level.getLevel(), spawnType);
...
//        }
//        //load segments
//        segmentCount = compound.getIntOr("SegmentCount", 0);
//        shouldScanForSegments = segmentCount > 0;
//        for (int i = 0; i < segmentCount; i++) {
//            String segmentUID = compound.getStringOr("SegmentUID_" + i, "");
...
//        spawnAtLocation(level, WDItems.EGG_SAC_ARROWS.toStack(UtilityMethods.RNG(8, 24)));
//    }
//
//    @Override
